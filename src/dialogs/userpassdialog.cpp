...
void UserPassDialog::Show(GtkWindow *parent, const std::string &service, const std::function<void(const std::string &, const std::string &)> &callback) {
  ...
  AdwAlertDialog *dialog = ADW_ALERT_DIALOG(adw_alert_dialog_new(title.c_str(), Translations::CStr("Enter username and password or token.")));
  GtkWidget *box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 8);
  GtkWidget *user = gtk_entry_new();
  gtk_entry_set_placeholder_text(GTK_ENTRY(user), Translations::CStr("Username"));
