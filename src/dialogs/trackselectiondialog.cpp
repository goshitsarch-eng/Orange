...
void TrackSelectionDialog::Show(GtkWindow *parent, Application *app, const SongList &songs) {
  ...
  GtkWidget *status = gtk_label_new(Translations::CStr("Searching AcoustID and MusicBrainz…"));
  ...
  app->tag_fetcher()->Results.Connect([status, list, app](const SongList &songs) {
    gtk_label_set_text(GTK_LABEL(status), songs.empty() ? Translations::CStr("No matches")
                                                       : (std::to_string(songs.size()) + Translations::Tr(" matches — click Apply to write tags")).c_str());
    for (const Song &song : songs) {
      ...
      g_signal_connect(apply, "clicked", G_CALLBACK(+[](GtkButton *btn, gpointer data) {
                         ...
                         gtk_button_set_label(btn, Translations::CStr("Applied"));
                       }),
                       app);
  ...
  if (!query.is_valid() && query.url().empty()) {
    gtk_label_set_text(GTK_LABEL(status), Translations::CStr("No song selected"));
    return;
  }
  if (songs.size() > 1) {
    gtk_label_set_text(GTK_LABEL(status),
                       (Translations::Tr("Searching tags for ") + std::to_string(songs.size()) + Translations::Tr(" selected tracks…")).c_str());
  }
