...
JsonLyricsProvider::JsonLyricsProvider(std::string name, std::string url_template) : name_(std::move(name)), url_template_(std::move(url_template)) {}
...
std::string JsonLyricsProvider::Extract(const std::string &body) const {
  return JsonUtils::ExtractLyrics(body);
}
