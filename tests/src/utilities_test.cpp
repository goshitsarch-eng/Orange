...
TEST(Transcoder, PresetAndPipelineFor) {
  ...
  EXPECT_NE(std::string::npos, speex.find("complexity="));
  TranscoderOptionsFields::Vorbis vorbis;
  vorbis.managed = true;
  vorbis.bitrate_bps = 192000;
  const std::string managed = vorbis.Pipeline();
  EXPECT_NE(std::string::npos, managed.find("managed=true"));
  EXPECT_NE(std::string::npos, managed.find("bitrate=192000"));
  TranscoderOptionsFields::Speex speex_opts;
  speex_opts.ApplyQuality(8);
  speex_opts.vbr = true;
  speex_opts.vad = true;
  const std::string speex_full = speex_opts.Pipeline();
  EXPECT_NE(std::string::npos, speex_full.find("vbr=true"));
